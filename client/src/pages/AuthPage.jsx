...
const AuthPage = () => {
    ...
    const handleGuestRegister = (e) => { e.preventDefault(); setDialog({ message: 'Guest registration will be built next!', type: 'info'}); };
        ...
        <div className="min-h-screen bg-gray-900 text-white flex items-center justify-center p-4">
            ...
            <div className="w-full max-w-5xl bg-gray-800 rounded-2xl shadow-2xl overflow-hidden flex flex-col md:flex-row">
                ...
                <div className="w-full md:w-1/2 p-8 md:p-12 bg-gray-900">
                    {/* JSX for login, register, OTP, and guest forms follows */}
                    {/* This code is long, but it correctly uses the state and handlers defined above */}
                        ...
                        <form onSubmit={handleLogin} className="space-y-6">
                            ...
                            <div>
                                ...
                                <div className="relative">
                                    <input id="login-password" name="password" type={showPassword ? 'text' : 'password'} value={loginData.password} onChange={handleLoginChange} placeholder="Password" required className="w-full px-4 py-2 bg-gray-700 border border-gray-600 rounded-lg text-white"/>
                                    <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute inset-y-0 right-0 px-3 flex items-center text-gray-400 hover:text-white">{showPassword ? <EyeOff size={20} /> : <Eye size={20} />}</button>
                                </div>
                            </div>
                            <button type="submit" disabled={isLoading} className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 px-4 rounded-lg transition disabled:opacity-50 flex items-center justify-center">{isLoading ? 'Signing In...' : <><LogIn className="mr-2" size={20} /> Sign In</>}</button>
                         ...
                         <form onSubmit={handlePhotographerRegister} className="space-y-4">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div>
                                    ...
                                    <select name="year" id="year" value={formData.year} onChange={handleFormChange} className="w-full px-4 py-2 bg-gray-700 border border-gray-600 rounded-lg text-white"><option value="">Select Year</option><option value="1">1st Year</option><option value="2">2nd Year</option><option value="3">3rd Year</option><option value="4">4th Year</option></select>
                                </div>
                                <div>
                                    <label htmlFor="department" className="block text-sm font-medium text-gray-300 mb-1">Department</label>
                                    <select name="department" id="department" value={formData.department} onChange={handleFormChange} className="w-full px-4 py-2 bg-gray-700 border border-gray-600 rounded-lg text-white"><option>B.Tech</option><option>M.Tech</option><option>BBA</option><option>BCA</option><option>LLB</option><option value="Other">Other</option></select>
                                </div>
                                {formData.department === 'Other' && (<FormInput id="otherDepartment" name="otherDepartment" type="text" value={formData.otherDepartment} onChange={handleFormChange} placeholder="Please specify department" />)}
                                ...
                                <FormInput id="lenses" name="lenses" type="text" value={formData.lenses} onChange={handleFormChange} placeholder="Lenses (e.g., 50mm)" isRequired={false} />
                                ...
                                <div>
                                    ...
                                    <select name="gender" id="gender" value={formData.gender} onChange={handleFormChange} className="w-full px-4 py-2 bg-gray-700 border border-gray-600 rounded-lg text-white"><option>Prefer not to say</option><option>Male</option><option>Female</option><option>Other</option></select>
                                </div>
                                ...
                                <div className="md:col-span-2">
                                    ...
                                    <div className="flex items-center space-x-2">
                                        <input id="username" name="username" type="text" value={formData.username} onChange={handleFormChange} placeholder="Create a unique username" required className="w-full px-4 py-2 bg-gray-700 border border-gray-600 rounded-lg text-white" />
                                    ...
                                    {usernameStatus === 'checking' && <p className="text-xs text-yellow-400 mt-1">Checking availability...</p>}
                                    {usernameStatus === 'available' && <p className="text-xs text-green-400 mt-1">Username is available!</p>}
                                    {usernameStatus === 'taken' && <p className="text-xs text-red-400 mt-1">{usernameError}</p>}
                                </div>
                                <div className="relative">
                                    <label htmlFor="password" className="block text-sm font-medium text-gray-300 mb-1">Password</label>
                                    <input id="password" name="password" type={showPassword ? 'text' : 'password'} value={formData.password} onChange={handleFormChange} placeholder="Create a Password" required className="w-full px-4 py-2 bg-gray-700 border border-gray-600 rounded-lg text-white" />
                                ...
                                <div className="relative">
                                    ...
                                    <input id="confirmPassword" name="confirmPassword" type={showConfirmPassword ? 'text' : 'password'} value={formData.confirmPassword} onChange={handleFormChange} placeholder="Confirm Password" required className="w-full px-4 py-2 bg-gray-700 border border-gray-600 rounded-lg text-white" />
                            ...
                            <button type="submit" disabled={isLoading || (formData.username && usernameStatus !== 'available')} className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 px-4 rounded-lg transition mt-4 flex items-center justify-center disabled:opacity-50">
                    ...
                    {authMode === 'registerGuest' && ( <form onSubmit={handleGuestRegister} className="space-y-6">{/* Guest Form JSX */} </form> )}
                </div>
            </div>
        </div>
    );
